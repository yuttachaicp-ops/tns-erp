...
function PixelBox({ children, title, color = '#00f5d4', style = {} }: { children: React.ReactNode; title?: string; color?: string; style?: React.CSSProperties }) {
  return (
    <div style={{ background: '#060d0b', border: `1px solid ${color}33`, position: 'relative', padding: title ? '28px 14px 14px' : '14px', boxShadow: `0 0 10px ${color}0a`, ...style }}>
      {['tl','tr','bl','br'].map(c => (
      ...
      {title && <div style={{ position: 'absolute', top: -1, left: 10, background: '#060d0b', padding: '0 6px', fontSize: '9px', color, fontFamily: '"Share Tech Mono", monospace', letterSpacing: '0.2em', fontWeight: 700 }}>{title}</div>}
      {children}
    </div>
  )
}
...
export default function PixelDashboard() {
  ...
  const PX = '"Share Tech Mono", monospace'
  ...
  return (
    <AppShell>
      ...
      <div className="px-scanlines" style={{ minHeight: '100vh', background: '#060d0b', padding: 16, fontFamily: PX }}>

        {/* TOP BAR */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#00f5d4', letterSpacing: '0.18em', textShadow: '0 0 14px rgba(0,245,212,0.6)' }}>TNS COMMAND CENTER</div>
            <div style={{ fontSize: 9, color: '#1a5a40', letterSpacing: '0.3em' }}>PIPELINE v2.0 — ISSUE TRACKER ONLINE</div>
          </div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ fontSize: 9, color: '#3a6a55' }}>{thDate}</span>
            <div style={{ background: '#0a1612', border: '1px solid #0d4a35', padding: '4px 12px', color: '#00f5d4', fontSize: 15, letterSpacing: '0.18em', textShadow: '0 0 8px rgba(0,245,212,0.5)' }}>⏱ {time || '--:--:--'}</div>
            {criticalCount > 0 && (
              <div style={{ background: 'rgba(255,68,68,0.1)', border: '1px solid rgba(255,68,68,0.4)', padding: '4px 10px', display: 'flex', alignItems: 'center', gap: 6, animation: 'px-warn 1.2s ease-in-out infinite' }}>
                ...
                <span style={{ fontSize: 9, color: '#ff4444', letterSpacing: '0.1em' }}>{criticalCount} CRITICAL ISSUE{criticalCount > 1 ? 'S' : ''}</span>
              </div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(0,255,136,0.07)', border: '1px solid rgba(0,255,136,0.2)', padding: '4px 10px' }}>
              <StatusDot />
              <span style={{ fontSize: 9, color: '#00ff88', letterSpacing: '0.12em' }}>ALL SYSTEMS ONLINE</span>
            </div>
          </div>
        </div>
            ...
            <PixelBox title="🐛 ISSUE TRACKER — PIPELINE PROBLEMS" color="#ff4444" style={{ minHeight: 400 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 14 }}>
                {[
                  ...
                  <div key={s.label} style={{ background: '#0a1612', border: `1px solid ${s.c}22`, padding: '8px', textAlign: 'center' }}>
                    <div style={{ fontSize: 8, color: '#3a6a55', letterSpacing: '0.12em', marginBottom: 3 }}>{s.label}</div>
              ...
              <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                {(['ALL','OPEN','IN_PROGRESS','RESOLVED'] as const).map(f => (
                  <button key={f} onClick={() => setFilter(f)} className="px-btn" style={{ padding: '4px 10px', fontSize: 9, fontFamily: PX, letterSpacing: '0.1em', cursor: 'pointer', background: filter === f ? 'rgba(0,245,212,0.15)' : 'transparent', border: `1px solid ${filter === f ? '#00f5d4' : '#0d2a1e'}`, color: filter === f ? '#00f5d4' : '#3a6a55', transition: 'all 0.15s' }}>
                    {f === 'IN_PROGRESS' ? 'IN PROG' : f}
                  </button>
                ))}
                <div style={{ flex: 1 }} />
                <button onClick={() => setShowAddModal(true)} className="px-btn" style={{ padding: '5px 14px', fontSize: 10, fontFamily: PX, letterSpacing: '0.1em', cursor: 'pointer', background: 'rgba(255,68,68,0.12)', border: '1px solid #ff4444', color: '#ff4444', fontWeight: 700 }}>
                  + REPORT ISSUE
                </button>
              </div>
              ...
              {/* Issue List */}
              {filtered.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '40px 20px', color: '#1a5a40', fontSize: 11, letterSpacing: '0.1em' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 56px 72px 70px 70px', gap: 8, padding: '4px 8px', borderBottom: '1px solid #0d2a1e', marginBottom: 2 }}>
                    {['SEV','ISSUE TITLE','STATUS','TIME','ACTION',''].map(h => (
                      <div key={h} style={{ fontSize: 8, color: '#1a5a40', letterSpacing: '0.15em' }}>{h}</div>
                    ))}
                  </div>
                  {filtered.map(issue => (
                    <div key={issue.id} className="issue-row" onClick={() => setShowIssueDetail(issue)}
                      style={{ display: 'grid', gridTemplateColumns: '60px 1fr 56px 72px 70px 70px', gap: 8, padding: '8px', background: 'transparent', borderBottom: '1px solid #0a1a12', alignItems: 'center', transition: 'background 0.1s' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        <span style={{ fontSize: 8, color: SEV_COLOR[issue.severity], letterSpacing: '0.08em' }}>{SEV_LABEL[issue.severity]}</span>
                      </div>
                      ...
                      <div>
                        ...
                        {issue.description && <div style={{ fontSize: 9, color: '#2a4a35', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 2 }}>{issue.description}</div>}
                      </div>
                      ...
                      <div style={{ background: `${STATUS_COLOR[issue.status]}15`, border: `1px solid ${STATUS_COLOR[issue.status]}44`, padding: '2px 4px', textAlign: 'center' }}>
                        <span style={{ fontSize: 8, color: STATUS_COLOR[issue.status], letterSpacing: '0.05em' }}>{STATUS_LABEL[issue.status]}</span>
                      </div>

                      {/* Time */}
                      <div style={{ fontSize: 9, color: '#2a4a35' }}>{timeAgo(issue.createdAt)}</div>
                      ...
                      <div onClick={e => e.stopPropagation()}>
                        <button onClick={() => deleteIssue(issue.id)} className="px-btn" title="Delete"
                          style={{ width: 22, height: 22, background: 'transparent', border: '1px solid #1a3a2a', color: '#2a4a35', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {[['PLAN','✓',true],['CODE','✓',true],['TEST','✓',true],['REVIEW','✓',true]].map(([l,ic,done]) => (
                  <div key={String(l)} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 9, color: done ? '#00ff88' : '#1a5a40', width: 40, fontFamily: PX }}>{l}</span>
                    <div style={{ flex: 1, height: 4, background: '#0a1612', border: '1px solid #0d2a1e', overflow: 'hidden' }}>
                ...
                <div style={{ marginTop: 4, textAlign: 'center', background: 'rgba(0,255,136,0.08)', border: '1px solid rgba(0,255,136,0.25)', padding: '5px', fontSize: 9, color: '#00ff88', letterSpacing: '0.15em' }}>
                  ► APPROVED ◄
                </div>
              </div>
            </PixelBox>
            ...
            <PixelBox title="DAILY TASKS" color="#00f5d4">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                {[['TODO', tasks.todo, '#ffd60a'], ['ACTIVE', tasks.inProgress, '#00f5d4'], ['DONE', tasks.done, '#00ff88'], ['TOTAL', totalTasks, '#c8f0e0']].map(([k, v, c]) => (
                  <div key={String(k)} style={{ background: '#0a1612', border: `1px solid ${c}22`, padding: '6px 8px', textAlign: 'center' }}>
                    <div style={{ fontSize: 8, color: '#3a6a55', letterSpacing: '0.1em', marginBottom: 2 }}>{k}</div>
            ...
            <PixelBox title="FINANCE" color="#ffd60a">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 8px', background: 'rgba(0,255,136,0.06)', border: '1px solid rgba(0,255,136,0.15)' }}>
                  <span style={{ fontSize: 9, color: '#3a6a55' }}>INCOME</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 8px', background: 'rgba(255,68,68,0.06)', border: '1px solid rgba(255,68,68,0.15)' }}>
                  <span style={{ fontSize: 9, color: '#3a6a55' }}>EXPENSE</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: '#ff6666' }}>฿{loaded ? fmt(expense) : '…'}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 8px', background: '#0a1612', border: '1px solid #0d4a35' }}>
                  <span style={{ fontSize: 9, color: '#3a6a55' }}>NET</span>
            ...
            <PixelBox title="QUEUES" color="#b44fff">
              {[['📷 PHOTO QUEUE', photoQ, '#ffd60a'], ['🛒 LISTING QUEUE', listingQ, '#b44fff']].map(([l, v, c]) => (
                <div key={String(l)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <span style={{ fontSize: 9, color: '#3a6a55' }}>{l}</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: String(c) }}>{loaded ? v : '…'}</span>
                </div>
              ))}
              <Link href="/dashboard" style={{ display: 'block', textAlign: 'center', fontSize: 9, color: '#b44fff', textDecoration: 'none', border: '1px solid #b44fff33', padding: '4px', background: 'rgba(180,79,255,0.06)', letterSpacing: '0.1em' }}>
                → FULL DASHBOARD
              </Link>
            </PixelBox>
          </div>
        ...
        <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 8 }}>
          {[
            ...
            <div key={s.label} style={{ background: '#080f0c', border: `1px solid ${s.c}22`, padding: '7px 10px', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                <div style={{ fontSize: 7, color: '#1a5a40', letterSpacing: '0.12em' }}>{s.label}</div>
      ...
      {/* ===== ADD ISSUE MODAL ===== */}
      {showAddModal && (
        <div onClick={() => setShowAddModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: PX }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#060d0b', border: '1px solid #ff444466', width: '100%', maxWidth: 460, position: 'relative', padding: 24, boxShadow: '0 0 40px rgba(255,68,68,0.15)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              ...
              <button onClick={() => setShowAddModal(false)} style={{ background: 'transparent', border: '1px solid #1a3a2a', color: '#3a6a55', cursor: 'pointer', fontSize: 12, padding: '2px 8px', fontFamily: PX }}>✕</button>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <div style={{ fontSize: 9, color: '#3a6a55', letterSpacing: '0.18em', marginBottom: 6 }}>ISSUE TITLE *</div>
                <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="ระบุปัญหาที่พบ..."
                  style={{ width: '100%', padding: '9px 12px', background: '#0a1612', border: '1px solid #0d4a35', color: '#c8f0e0', fontSize: 12, fontFamily: PX, outline: 'none', boxSizing: 'border-box' as const }}
                  onFocus={e => e.target.style.borderColor = '#ff4444'}
                  onBlur={e => e.target.style.borderColor = '#0d4a35'} />
              </div>

              <div>
                <div style={{ fontSize: 9, color: '#3a6a55', letterSpacing: '0.18em', marginBottom: 6 }}>DESCRIPTION (OPTIONAL)</div>
                <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} rows={3} placeholder="รายละเอียดเพิ่มเติม..."
                  style={{ width: '100%', padding: '9px 12px', background: '#0a1612', border: '1px solid #0d4a35', color: '#c8f0e0', fontSize: 12, fontFamily: PX, outline: 'none', resize: 'vertical', boxSizing: 'border-box' as const }}
                  onFocus={e => e.target.style.borderColor = '#ff4444'}
                  onBlur={e => e.target.style.borderColor = '#0d4a35'} />
              </div>

              <div>
                <div style={{ fontSize: 9, color: '#3a6a55', letterSpacing: '0.18em', marginBottom: 6 }}>SEVERITY</div>
                <div style={{ display: 'flex', gap: 6 }}>
                  {(['LOW','MEDIUM','HIGH','CRITICAL'] as const).map(s => (
                    <button key={s} onClick={() => setNewSev(s)} className="px-btn"
                      style={{ flex: 1, padding: '6px 4px', fontSize: 9, fontFamily: PX, cursor: 'pointer', background: newSev === s ? `${SEV_COLOR[s]}18` : 'transparent', border: `1px solid ${newSev === s ? SEV_COLOR[s] : '#0d2a1e'}`, color: newSev === s ? SEV_COLOR[s] : '#3a6a55', letterSpacing: '0.05em', transition: 'all 0.15s' }}>
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <button onClick={addIssue} disabled={!newTitle.trim() || adding} className="px-btn"
                style={{ padding: '10px', background: newTitle.trim() ? 'rgba(255,68,68,0.12)' : 'transparent', border: `1px solid ${newTitle.trim() ? '#ff4444' : '#1a3a2a'}`, color: newTitle.trim() ? '#ff4444' : '#3a6a55', fontSize: 11, fontWeight: 700, cursor: newTitle.trim() ? 'pointer' : 'not-allowed', letterSpacing: '0.15em', fontFamily: PX, transition: 'all 0.15s' }}>
      ...
      {/* ===== ISSUE DETAIL MODAL ===== */}
      {showIssueDetail && (
        <div onClick={() => setShowIssueDetail(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: PX }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#060d0b', border: `1px solid ${STATUS_COLOR[showIssueDetail.status]}44`, width: '100%', maxWidth: 480, position: 'relative', padding: 24, boxShadow: `0 0 40px ${STATUS_COLOR[showIssueDetail.status]}15` }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <span style={{ fontSize: 10, color: '#3a6a55', letterSpacing: '0.15em' }}>ISSUE DETAIL</span>
              <button onClick={() => setShowIssueDetail(null)} style={{ background: 'transparent', border: '1px solid #1a3a2a', color: '#3a6a55', cursor: 'pointer', fontSize: 12, padding: '2px 8px', fontFamily: PX }}>✕</button>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 8, color: SEV_COLOR[showIssueDetail.severity], letterSpacing: '0.12em' }}>SEVERITY: {showIssueDetail.severity}</span>
                <div style={{ width: 1, height: 10, background: '#0d2a1e' }} />
                <span style={{ fontSize: 8, color: STATUS_COLOR[showIssueDetail.status], letterSpacing: '0.1em' }}>STATUS: {showIssueDetail.status}</span>
              </div>
              <div style={{ fontSize: 14, color: '#c8f0e0', marginBottom: 8 }}>{showIssueDetail.title}</div>
              {showIssueDetail.description && <div style={{ fontSize: 11, color: '#3a6a55', background: '#0a1612', padding: '8px 10px', border: '1px solid #0d2a1e', lineHeight: 1.6 }}>{showIssueDetail.description}</div>}
              <div style={{ fontSize: 8, color: '#1a5a40', marginTop: 8 }}>REPORTED: {new Date(showIssueDetail.createdAt).toLocaleString('th-TH')}</div>
              {showIssueDetail.resolvedAt && <div style={{ fontSize: 8, color: '#00ff8866', marginTop: 2 }}>RESOLVED: {new Date(showIssueDetail.resolvedAt).toLocaleString('th-TH')}</div>}
            </div>

            <div style={{ fontSize: 9, color: '#3a6a55', letterSpacing: '0.15em', marginBottom: 8 }}>UPDATE STATUS</div>
            <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
              {(['OPEN','IN_PROGRESS','RESOLVED'] as const).map(s => (
                <button key={s} onClick={() => { updateStatus(showIssueDetail.id, s); setShowIssueDetail({ ...showIssueDetail, status: s }) }} className="px-btn"
                  style={{ flex: 1, padding: '7px', fontSize: 9, fontFamily: PX, cursor: 'pointer', background: showIssueDetail.status === s ? `${STATUS_COLOR[s]}18` : 'transparent', border: `1px solid ${showIssueDetail.status === s ? STATUS_COLOR[s] : '#0d2a1e'}`, color: showIssueDetail.status === s ? STATUS_COLOR[s] : '#3a6a55', letterSpacing: '0.06em', transition: 'all 0.15s' }}>
                  {STATUS_LABEL[s]}
                </button>
              ))}
            </div>
            ...
            <button onClick={() => deleteIssue(showIssueDetail.id)} className="px-btn"
              style={{ width: '100%', padding: '6px', background: 'transparent', border: '1px solid #1a3a2a', color: '#3a6a55', fontSize: 9, cursor: 'pointer', letterSpacing: '0.12em', fontFamily: PX, transition: 'all 0.15s' }}
              onMouseEnter={e => { (e.currentTarget.style.borderColor = '#ff4444'); (e.currentTarget.style.color = '#ff4444') }}
