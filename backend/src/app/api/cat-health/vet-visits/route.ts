...
export async function GET(req:NextRequest){
  ...
  const items=await prisma.catVetVisit.findMany({where:{userId:u.userId},orderBy:{visitDate:'desc'}})
...
export async function POST(req:NextRequest){
  ...
  const item=await prisma.catVetVisit.create({data:{
    userId:u.userId,visitDate:b.visitDate?new Date(b.visitDate):new Date(),
